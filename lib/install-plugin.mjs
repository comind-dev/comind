// Stage 1: `npx @comind-dev/comind` — INSTALL ONLY.
//
 ...
 * an entry not present in the shipped bin/. So `npx @comind-dev/comind`, documented as safe
 ...
 * `npx @comind-dev/comind` into a silent no-op on any machine that had ever run
...
export function renderInstallNextSteps(report, versions) {
  ...
  if (viaPlugin) {
    ...
    if (report.fallbackReason !== 'forced') {
      L.push('             Re-run `npx @comind-dev/comind` once the claude CLI is on PATH to');
